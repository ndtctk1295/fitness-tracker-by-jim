...
export function useExerciseOrder({
  ...
  const getOrderedExercises = useCallback(() => {
    ...
    // Find any exercises not in the ordered list (shouldn't happen with our new useEffect, but just in case)
  ...
  }, [activeTimer.currentStrategyId]);  // Ensure ordered exercises are initialized before starting timer
